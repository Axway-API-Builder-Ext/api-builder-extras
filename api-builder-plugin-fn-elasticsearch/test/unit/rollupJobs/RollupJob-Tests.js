...
describe('Rollup Job tests', () => {
	...
	describe('#Get Rollup jobs tests', () => {
		...
		it('should follow notFound with an invalid Job ID', async () => {
			const mockedFn = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getRollupJobsNotFoundResponse.json', false);
		...
		it('should pass with a valid Job ID', async () => {
			const mockedFn = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getRollupJobsResponse.json', false);
	...
	describe('#Put Rollup job tests', () => {
		...
		it('should fail if the same job-id is re-used, but no ID-Suffix is given.', async () => {
			const mockedGetRollupJob = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getRollupJobsResponse.json', false);
			const mockedPutRollupJob = setupElasticsearchMock(client, 'rollup.putJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);

			const inputParameter = { id: 'traffic-summary-rollup-job', body: JSON.parse(fs.readFileSync('./test/unit/mock/rollupJobs/putRollupJobRequestBody.json')) };
			const { value, output } = await flowNode.putRollupJob(inputParameter);
		...
		it('should pass - with 2 running jobs (means no actual) - Created & Started new Job', async () => {
			const mockedGetRollupJobs = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getTwoStartedRollupJobsResponse.json', false);
			const mockedPutRollupJob = setupElasticsearchMock(client, 'rollup.putJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedDeleteRollupJob = setupElasticsearchMock(client, 'rollup.deleteJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedStartRollupJob = setupElasticsearchMock(client, 'rollup.startJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedStopRollupJob = setupElasticsearchMock(client, 'rollup.stopJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);

			const inputParameter = { 
				id: 'traffic-summary-rollup', 
				idSuffix: "v1",
				body: JSON.parse(fs.readFileSync('./test/unit/mock/rollupJobs/putRollupJobRequestBody.json')) };
			const { value, output } = await flowNode.putRollupJob(inputParameter);
		...
		it('should pass - 1 running job (is the actual job) - Created & Started new Job - Old stopped', async () => {
			const mockedGetRollupJobs = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getOneStartedRollupJobsResponse.json', false);
			const mockedPutRollupJob = setupElasticsearchMock(client, 'rollup.putJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedDeleteRollupJob = setupElasticsearchMock(client, 'rollup.deleteJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedStartRollupJob = setupElasticsearchMock(client, 'rollup.startJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedStopRollupJob = setupElasticsearchMock(client, 'rollup.stopJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
		...
		it('should pass with valid parameters - Job-ID not found - Will just create new Job', async () => {
			const mockedFn = setupElasticsearchMock(client, 'rollup.putJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);

			const inputParameter = { id: 'myTestJob', body: JSON.parse(fs.readFileSync('./test/unit/mock/rollupJobs/putRollupJobRequestBody.json')) };
			const { value, output } = await flowNode.putRollupJob(inputParameter);
		...
		it('should update/recreate the job if configuration is changed', async () => {
			...
			var rollupJobConfig = JSON.parse(fs.readFileSync('./test/unit/mock/rollupJobs/putRollupJobRequestBody.json'))
		...
		it('should update/recreate the job if not yet exists', async () => {
			const mockedDeleteRollupJob = setupElasticsearchMock(client, 'rollup.deleteJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedPutRollupJob = setupElasticsearchMock(client, 'rollup.putJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
			const mockedGetRollupJob = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getRollupJobsNotFoundResponse.json', false);
			const mockedStartRollupJob = setupElasticsearchMock(client, 'rollup.startJob', './test/unit/mock/rollupJobs/putRollupJobResponse.json', false);
		...
		it('should gracefully handle the JOB-ID already exists response', async () => {
			const mockedPutRollupJob = setupElasticsearchMock(client, 'rollup.putJob', './test/unit/mock/rollupJobs/putRollupJobAlreadyExistsResponse.json', true);
			const mockedGetRollupJob = setupElasticsearchMock(client, 'rollup.getJobs', './test/unit/mock/rollupJobs/getRollupJobsNotFoundResponse.json', false);
