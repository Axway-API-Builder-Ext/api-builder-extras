...
describe('Index document tests', () => {
	...
	describe('#Index document actions', () => {
		...
		it('should pass without any optional parameter', async () => {
			const mockedFn = setupElasticsearchMock(client, 'index', './test/unit/mock/indexDocument/document-indexed-response.json');
		...
		it('should fail if index does not exists and cannot be created', async () => {
			const mockedFn = setupElasticsearchMock(client, 'index', './test/unit/mock/indexDocument/missing-index-response.json');
