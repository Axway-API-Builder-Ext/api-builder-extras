...
describe('ILM Policy tests', () => {
	...
	describe('#Get ILM Policy tests', () => {
		...
		it('should follow notFound with an invalid policy name', async () => {
			const mockedFn = setupElasticsearchMock(client, 'ilm.getLifecycle', './test/unit/mock/ilm/getILMPolicyNotFoundResponse.json', false);
		...
		it('should pass with a valid policy-name and body', async () => {
			const mockedFn = setupElasticsearchMock(client, 'ilm.getLifecycle', './test/unit/mock/ilm/getILMPolicyResponse.json', false);
	...
	describe('#Put ILM policy actions', () => {
		...
		it('should pass without with valid parameters', async () => {
			const mockedFn = setupElasticsearchMock(client, 'ilm.putLifecycle', './test/unit/mock/ilm/putILMPolicyResponse.json', false);
			const mockedGetTemplate = setupElasticsearchMock(client, 'indices.getTemplate', './test/unit/mock/indexTemplates/getTemplateResponse.json', false);
		...
		it('should return with noUpate, if updateWhenChanged is given and policies are equal', async () => {
			...
			const mockedGetILMPolicy = setupElasticsearchMock(client, 'ilm.getLifecycle', './test/unit/mock/ilm/getILMPolicyResponse.json', false);

			const inputParameter = { 
				policy: 'test-ilm-policy', 
				body: JSON.parse(fs.readFileSync('./test/unit/mock/ilm/putILMPolicyRequestBody.json')), 
		...
		it('should update the ILM-Policy if given policy is different', async () => {
			// This method is used by the implementation to load the actual ILM-Policy
			const mockedGetILMPolicy = setupElasticsearchMock(client, 'ilm.getLifecycle', './test/unit/mock/ilm/getILMPolicyResponse.json', false);
			const mockedPutILMPolicy = setupElasticsearchMock(client, 'ilm.putLifecycle', './test/unit/mock/ilm/putILMPolicyResponse.json', false);

			var testILMPolicy = JSON.parse(fs.readFileSync('./test/unit/mock/ilm/putILMPolicyRequestBody.json'));
		...
		it('should create the ILM-Policy if the actual policy does not exists', async () => {
			// This method is used by the implementation to load the actual ILM-Policy
			const mockedGetILMPolicy = setupElasticsearchMock(client, 'ilm.getLifecycle', './test/unit/mock/ilm/getILMPolicyNotFoundResponse.json', false);
			const mockedPutILMPolicy = setupElasticsearchMock(client, 'ilm.putLifecycle', './test/unit/mock/ilm/putILMPolicyResponse.json', false);
		...
		it('should return with noUpate, even if the structure of the given objects is slightly different and default are not given', async () => {
			...
			const mockedGetILMPolicy = setupElasticsearchMock(client, 'ilm.getLifecycle', './test/unit/mock/ilm/getILMPolicyResponse2.json', false);

			const inputParameter = { 
				policy: 'test-ilm-policy', 
				body: JSON.parse(fs.readFileSync('./test/unit/mock/ilm/putILMPolicyRequestBody2.json')), 
				updateWhenChanged: true
			 };
			const { value, output } = await flowNode.putILMPolicy(inputParameter);
		...
		it('should attach the ILM-Policy if the Index-Template has no ILM-Policy attached yet', async () => {
			...
			const mockedPutILMPolicy = setupElasticsearchMock(client, 'ilm.putLifecycle', './test/unit/mock/ilm/putILMPolicyResponse.json', false);
			const mockedPutTemplate = setupElasticsearchMock(client, 'indices.putTemplate', './test/unit/mock/indexTemplates/putTemplateResponse.json', false);
			const mockedGetTemplate = setupElasticsearchMock(client, 'indices.getTemplate', './test/unit/mock/indexTemplates/getTemplateResponseNoILMPolicy.json', false);
